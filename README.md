...
7 クラス分類（チャンスレベル ≈ 14.3%）における、動作開始時点を 0 ms とした
各時点での分類精度（被験者 5 名の平均）：

| 手法 | −300 ms | −100 ms | 0 ms | +200 ms | +400 ms |
|------|:------:|:------:|:----:|:------:|:------:|
| EEG のみ | 0.40 | 0.48 | 0.46 | 0.50 | 0.49 |
| EMG + モーションのみ | 0.26 | 0.45 | 0.64 | 0.77 | 0.78 |
| EEG + EMG + モーション | 0.29 | 0.49 | 0.65 | 0.77 | 0.78 |

- **EMG + モーション**は動作開始前（−300 ms）ではチャンス付近だが、動作が始まると
  急激に精度が上がる。運動そのものの情報しか持たないため。
- **EEG のみ**は −300 ms の時点でも 0.40 と、動作開始前から意図に関する情報を
  保持している。時間方向の変化は小さい。
- 動作開始前の窓（−300 / −100 ms）では、融合モデルが EMG + モーション単独を
  上回る。先読みの局面では EEG が寄与する。
