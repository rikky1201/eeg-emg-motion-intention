...
| 手法 | 帯域分割 → スライディング窓 → 特徴量抽出（Hjorth / CSP / RMS / 運動学） → 全結合 NN |
...
│   ├── features.py             #   Hjorth / CSP→Hjorth / RMS / 平均位置 / 環境 特徴量
...
- **マルチモーダル融合** — EEG・EMG・モーションの特徴量を結合し、単一モダリティより
  早く・安定して意図を推定する。
- **CSP 空間フィルタ** — 静止 vs 動作の 2 値で学習した Common Spatial Patterns で
  運動関連の脳波成分を強調し、その成分に Hjorth パラメータを適用。
- **Hjorth パラメータ** — 分散ベースの軽量な特徴量（activity / mobility）で
  リアルタイム推論に必要な計算コストを抑える。
